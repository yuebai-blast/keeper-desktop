...
config/settings.py 部署/运行配置，收口全部环境变量
...
| 模型 | 用途 | 落盘位置（默认 `~/.cache/keeper/models/`） |
| :-- | :-- | :-- |
| DINOv2 | 分组语义特征 | `huggingface/hub/` |
| InsightFace `buffalo_l` | 人脸检测/关键点/识别 | `insightface/` |
| TOPIQ-nr / TOPIQ-nr-face / CLIP-IQA+ | 画质/美学 | `torch/` |

- 根目录可用 `KEEPER_MODELS_DIR` 覆盖。设备默认 CPU（桌面最稳），`KEEPER_DEVICE=cuda` 走 CUDA；pyiqa 在 MPS 易炸，固定不走 MPS。
...
| **API key 本地管理** | 存 `~/.config/keeper/ark_key`（0600）或环境变量，绝不入库 |
...
| `KEEPER_MODELS_DIR` | 模型缓存根（默认 `~/.cache/keeper/models`） |
| `KEEPER_THUMBS_DIR` | 缩略图缓存（默认 `~/.cache/keeper/thumbnails`） |
| `KEEPER_DINO_MODEL` / `KEEPER_FACE_PACK` | 切分组/人脸模型（默认 `facebook/dinov2-small` / `buffalo_l`） |
| `ARK_API_KEY` | 大模型 key（或写入 `~/.config/keeper/ark_key`，0600） |
