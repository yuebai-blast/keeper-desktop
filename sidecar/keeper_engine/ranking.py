...
漏斗规则本身是两层通用的 `apply_funnel`（见 funnel.py）；这里在它之上加一层
PK 语义：给每张标注「达标通过 / 兜底补入」的来源，供前端透明展示去留理由。
...
from .models import PkEntry, PkOrigin, PkSet, Score


def assemble_pk_set(group_id: str, scores: list[Score], n: int) -> PkSet:
    ...
    本函数只负责把结果翻译成带来源标注的 `PkEntry`。
    返回 entries 数 = min(K, max(达标数, N))。
    """
    entries = [
        PkEntry(
            path=s.path,
        ...
        for s, is_quota_fill in apply_funnel(scores, n)
    ]
