...
设计：
  ...
  - 设备：默认 CPU（桌面端最稳）；KEEPER_DEVICE=cuda 时用 CUDA。pyiqa 在 MPS 上易炸，固定不走 MPS。
"""
...
logger = logging.getLogger("keeper_engine.vision")
...
def _models_root() -> Path:
    root = Path(os.environ.get("KEEPER_MODELS_DIR", Path.home() / ".cache" / "keeper" / "models"))
...
def _torch_device():
    ...
    forced = os.environ.get("KEEPER_DEVICE", "").lower()
    if forced == "cuda":
...
def _ensure_pyiqa(key: str, metric_name: str, label: str):
    _ensure_storage_configured()
    if key in _models:
        return _models[key]
    with _LOCK:
...
def _pyiqa_score(key: str, metric_name: str, label: str, img: Image.Image) -> float:
    import torch

    model = _ensure_pyiqa(key, metric_name, label)
    with torch.no_grad():
        score = model(_resize_for_pyiqa(img))
    return float(score.item() if hasattr(score, "item") else score)


def topiq_score(img: Image.Image) -> float:
    """TOPIQ-nr 通用技术质量分（约 0–1，越高越好）。用于无脸照（风景/空镜）。"""
    return _pyiqa_score("topiq", "topiq_nr", "TOPIQ-nr", img)


def topiq_face_score(face_img: Image.Image) -> float:
    """TOPIQ-nr-face 人脸质量分（GFIQA 训练，约 0–1，越高越好）。

    专为人脸场景设计，输入应为人脸裁剪。人像选片用它比通用 topiq_nr 更贴合。
    """
    return _pyiqa_score("topiq_face", "topiq_nr-face", "TOPIQ-nr-face", face_img)


def clipiqa_plus_score(img: Image.Image) -> float:
    """CLIP-IQA+ 美学分（约 0–1，越高越好）。"""
    return _pyiqa_score("clipiqa", "clipiqa+", "CLIP-IQA+", img)
...
def _face_pack() -> str:
    """人脸模型包名。默认 buffalo_l（高精度，效果优先）；可用 KEEPER_FACE_PACK 切轻包。"""
    return os.environ.get("KEEPER_FACE_PACK", "buffalo_l")


def _ensure_insightface(modules: tuple[str, ...] = DETECT_MODULES):
    _ensure_storage_configured()
    ...
    with _LOCK:
        ...
        pack = _face_pack()
        root = str(_models_root() / "insightface")
...
def extract_faces(
    img: Image.Image, max_dim: int = 1024, modules: tuple[str, ...] = DETECT_MODULES
) -> list[dict]:
    ...
    """
    app = _ensure_insightface(modules)
    rgb = img.convert("RGB")
    w, h = rgb.size
    scale = 1.0
    if max(w, h) > max_dim:
...
def require_layer1_capabilities() -> None:
